...
{
  private static const PX4:UnitValue = UnitValue.create(4, UnitValue.PIXEL);
	private static const PX6:UnitValue = UnitValue.create(6, UnitValue.PIXEL);
	private static const PX7:UnitValue = UnitValue.create(7, UnitValue.PIXEL);
	private static const PX9:UnitValue = UnitValue.create(9, UnitValue.PIXEL);
	private static const PX10:UnitValue = UnitValue.create(10, UnitValue.PIXEL);
	private static const PX11:UnitValue = UnitValue.create(11, UnitValue.PIXEL);
	private static const PX12:UnitValue = UnitValue.create(12, UnitValue.PIXEL);
	private static const PX14:UnitValue = UnitValue.create(14, UnitValue.PIXEL);
	private static const PX16:UnitValue = UnitValue.create(16, UnitValue.PIXEL);
	private static const PX18:UnitValue = UnitValue.create(18, UnitValue.PIXEL);
	private static const PX20:UnitValue = UnitValue.create(20, UnitValue.PIXEL);
...
//	private static final int KDE = 3;
	...
	private static const PANEL_INS:Vector.<UnitValue> = new UnitValue[4];
	private static const DIALOG_INS:Vector.<UnitValue> = new UnitValue[4];
	...
	static var RELATED_X:BoundSize;
  static var RELATED_Y:BoundSize;
  static var UNRELATED_X:BoundSize;
  static var UNRELATED_Y:BoundSize;
   ...
   * Returns the platform is running on currently.
	...
	public static function setPlatform(plaf:int):void
	{
		switch (plaf) {
			case WINDOWS_XP:
				setRelatedGap(PX4, PX4);
				setUnrelatedGap(PX7, PX7);
				setParagraphGap(PX14, PX14);
				setIndentGap(PX9, PX9);
				setGridCellGap(PX4, PX4);

				setMinimumButtonWidth(UnitValue.create(75, UnitValue.PIXEL));
				buttonOrder = "L_E+U+YNBXOCAH_R";
				setDialogInsets(PX11, PX11, PX11, PX11);
				setPanelInsets(PX7, PX7, PX7, PX7);
				break;

			case MAC_OSX:
				setRelatedGap(PX4, PX4);
				setUnrelatedGap(PX7, PX9);
				setParagraphGap(PX14, PX14);
				setIndentGap(PX10, PX10);
				setGridCellGap(PX4, PX4);

				setMinimumButtonWidth(UnitValue.create(68, UnitValue.PIXEL));
				buttonOrder = "L_HE+U+NYBXCOA_R";
				setDialogInsets(PX14, PX20, PX20, PX20);
				setPanelInsets(PX16, PX16, PX16, PX16);
				break;

			case GNOME:
				setRelatedGap(PX6, PX6);                    // GNOME HIG 8.2.3
				setUnrelatedGap(PX12, PX12);                // GNOME HIG 8.2.3
				setParagraphGap(PX18, PX18);                // GNOME HIG 8.2.3
				setIndentGap(PX12, PX12);                   // GNOME HIG 8.2.3
				setGridCellGap(PX6, PX6);                   // GNOME HIG 8.2.3

				// GtkButtonBox, child-min-width property default value
				setMinimumButtonWidth(UnitValue.create(85, UnitValue.PIXEL));
				buttonOrder = "L_HE+UNYACBXIO_R";           // GNOME HIG 3.4.2, 3.7.1
				setDialogInsets(PX12, PX12, PX12, PX12);  // GNOME HIG 3.4.3
				setPanelInsets(PX6, PX6, PX6, PX6);       // ???
				break;
		...
		CUR_PLAF = plaf;
		BASE_DPI = BASE_DPI_FORCED != -1 ? BASE_DPI_FORCED : 96 /* метод getPlatformDPI удален, всегда 96 */;
	}
  ...
  public static function getDefaultDPI():int {
    return BASE_DPI;
  }
	 ...
	 */
	public static function setDefaultDPI(dpi:int):void
	{
	 ...
	 */
	public static function getHorizontalScaleFactor():Number
	{
		return horScale;
	}
	 ...
	 * @see ComponentWrapper# getHorizontalScreenDPI()
	 */
	public static function setHorizontalScaleFactor(f:Number):void
	{
		if (horScale != f) {
			horScale = f;
			_modCount++;
...
//	}
  ...
  public static function setGridCellGap(x:UnitValue, y:UnitValue):void {
	 ...
	 */
  public static function setMinimumButtonWidth(width:UnitValue):void {
    BUTT_WIDTH = width;
    _modCount++;
  }
   ...
   * Returns the recommended minimum button width depending on the current set platform.
	 ...
	 */
  public static function getMinimumButtonWidth():UnitValue {
    return BUTT_WIDTH;
  }
  ...
  public static function setUnitValue(unitStrings:Vector.<String>, x:UnitValue, y:UnitValue):void {
    for (var i:int = 0, n:int = unitStrings.length; i < n; i++) {
      const s:String = unitStrings[i].toLowerCase();
      if (x != null) {
        HOR_DEFS[s] = x;
      }
      if (y != null) {
	 ...
	 */
	static function convertToPixels(value:Number, unit:String, isHor:Boolean, ref:Number, parent:ContainerWrapper, comp:ComponentWrapper):int
	{
  ...
  private static var _buttonOrder:String;
  ...
  public static function get labelAlignPercentage():Number {
	...
	{
		if (_gapProvider != null) {
      return _gapProvider.getDefaultGap(comp, adjacentComp, adjacentSide, tag, isLTR);
    }
		if (adjacentComp == null) {
      return null;
    }
		return (adjacentSide == CocoaConstants.LEFT || adjacentSide == CocoaConstants.RIGHT) ? RELATED_X : RELATED_Y;
	}

  private static var _gapProvider:InCellGapProvider;
	/**
  ...
  private static var _modCount:int = 0;
	/**
  ...
  private static var _defaultHorizontalUnit:int = UnitValue.PIXEL;
  ...
  public static function get defaultHorizontalUnit():int {
	...
	public static function set defaultHorizontalUnit(unit:int)
	{
      ...
      throw new ArgumentError("Illegal Unit: " + unit);
		...
		if (_defaultHorizontalUnit != unit) {
			_defaultHorizontalUnit = unit;
			_modCount++;
		}
	}

  private static var _defaultVerticalUnit:int = UnitValue.PIXEL;
  /** Returns the current default unit. The default unit is the unit used if no unit is set. E.g. "width 10".
   * @return The current default unit.
   * @see UnitValue #PIXEL
   * @see UnitValue #LPY
  ...
  public static function set defaultVerticalUnit(unit:int):void {
    ...
    if (_defaultVerticalUnit != unit) {
      _defaultVerticalUnit = unit;
      _modCount++;
    }
  }
